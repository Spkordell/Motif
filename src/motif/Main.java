...
public class Main {
    ...
    private static void createAndShowGUI() {
        ...
        //LinkedList<Float> patternStrengths;
        for (String elem : arr) {
        	patterns = new LinkedList<String>();
        	//patternStrengths = new LinkedList<Float>();
	        ...
	        for (int i = 2; i < elem.length()/2; i+=1){
	           ...
	           if (matcher.find()) {
	        	  ...
	        	  //patternStrengths.add(determinePatternStrength(elem,matcher.group(1).trim()));
	        	  int j = 0;
	        	  do {
		        	  j = elem.indexOf(' ',++j);
     	   ...
     	   for(String pattern : patterns){
     		   System.out.println("Pattern "+(index++)+": "+pattern/*+" = "+patternStrengths.get(index-1)*100+"%"*/);
     	   }
        	...
        	for(String pattern : patterns) {
        		...
        		if (matcher.find()) {
        			...
        			int idx = pattern.length();
                 	...
                 	Float predictionStrength = determinePredictionStrength(elem, pattern, prediction);
        	...
        	//Select a prediction (first, find prediction with highest accuracy. Break ties by selecting prediction with most elements.
          	if (predictions.size() > 0) {
          		int predictionIndex = 0;
    ...
    private static float determinePatternStrength(String elem, String pattern) {
    	//Much more could probably go into this calculation. Currently it only compares the number of times the pattern occurs with and without the last element. Once could probably add many other comparisons to get a better value. 
